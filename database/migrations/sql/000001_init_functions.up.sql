...
$$
WITH RECURSIVE
    objects_cte as (select object_id, parent_id, owner_id
                    from object
    ...
    object_users_cte AS (SELECT user_id, objects_cte.object_id AS object_id
                         FROM user_object
                                  INNER JOIN objects_cte ON user_object.object_id = objects_cte.object_id
                         WHERE user_object.value ->> 'role' = 'admin'
                         union all
                         SELECT owner_id as user_id, object_id
                         from objects_cte),
    users_cte as (select user_id
...
AS
$$
with recursive cte as (select object_id,
                              parent_id,
                              0 as level
                       from object
                       where object_id = t_pid
                       ...
                       where level < t_lev
                         and cte.parent_id != cte.object_id)
select object_id,
       parent_id,
       level
from cte;
