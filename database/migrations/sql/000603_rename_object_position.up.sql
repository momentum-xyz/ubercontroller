...
ALTER TABLE "object"
  RENAME COLUMN "position" TO "transform";

-- create replace does not allow renames of cols
-- and then we need to drop the func that uses it first.
...
CREATE OR REPLACE VIEW objects_plus_level_isadmin
            ...
            options, transform, updated_at, created_at, level, is_admin)
AS
