...
IF EXISTS (SELECT 1 FROM attribute_type WHERE attribute_name = 'canvas_contribution') THEN
    DELETE FROM attribute_type WHERE attribute_name = 'canvas_contribution';
    RAISE NOTICE 'Deleted attribute_type with name: canvas_contribution';
END IF;

IF EXISTS (SELECT 1 FROM attribute_type WHERE attribute_name = 'canvas') THEN
    DELETE FROM attribute_type WHERE attribute_name = 'canvas';
    RAISE NOTICE 'Deleted attribute_type with name: canvas';
END IF;
