...
func (n *Node) apiGetUserAttributeValue(c *gin.Context) {
	...
	object, ok := n.GetUserFromAllUsers(userID)
	if !ok {
		err := errors.Errorf("Node: apiGetUserAttributeValue: user not found: %s", userID)
	...
	out, ok := user.GetUserAttributes().GetValue(attributeID)
	if !ok {
