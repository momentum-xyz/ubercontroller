...
func (uo *userObjects) GetObjectIndirectAdmins(objectID mid.ID) ([]*mid.ID, bool) {
...
func (uo *userObjects) CheckIsIndirectAdmin(userObjectID entry.UserObjectID) (bool, error) {
	...
	if err != nil {
		return false, errors.WithMessage(err, "failed to check is indirect admin by mid")
	}
	return isAdmin, nil
}
...
func (uo *userObjects) Remove(userObjectID entry.UserObjectID, updateDB bool) (bool, error) {
	if err := uo.node.db.GetUserObjectsDB().RemoveUserObjectByID(uo.node.ctx, userObjectID); err != nil {
		...
		return false, errors.WithMessage(err, "failed to remove user object by mid")
	}
	return true, nil
}
