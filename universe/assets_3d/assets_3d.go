...
type Assets3d struct {
	...
	assets *generic.SyncMap[universe.AssetUserIDPair, universe.Asset3d]
...
func NewAssets3d(db database.DB) *Assets3d {
	return &Assets3d{
		...
		assets: generic.NewSyncMap[universe.AssetUserIDPair, universe.Asset3d](0),
	}
}
...
func (a *Assets3d) CreateAsset3d(asset3dID universe.AssetUserIDPair) (universe.Asset3d, error) {
...
func (a *Assets3d) FilterAssets3d(predicateFn universe.Assets3dFilterPredicateFn) map[universe.AssetUserIDPair]universe.Asset3d {
	return a.assets.Filter(predicateFn)
}

func (a *Assets3d) GetAsset3d(assetUserID universe.AssetUserIDPair) (universe.Asset3d, bool) {
	// TODO add userID
	asset, ok := a.assets.Load(assetUserID)
	return asset, ok
}

func (a *Assets3d) GetAssets3d() map[universe.AssetUserIDPair]universe.Asset3d {
	a.assets.Mu.RLock()
	defer a.assets.Mu.RUnlock()

	assets := make(map[universe.AssetUserIDPair]universe.Asset3d, len(a.assets.Data))
...
func (a *Assets3d) AddAsset3d(asset3d universe.Asset3d, updateDB bool) error {
	...
	a.assets.Data[asset3d.GetUserAssetIDPair()] = asset3d
...
func (a *Assets3d) AddAssets3d(assets3d []universe.Asset3d, updateDB bool) error {
	...
	for i := range assets3d {
		// TODO combine Asset3dID and UserID to create a unique key?
		a.assets.Data[assets3d[i].GetUserAssetIDPair()] = assets3d[i]
	}
...
func (a *Assets3d) RemoveAsset3d(asset3d universe.Asset3d, updateDB bool) (bool, error) {
	...
	if _, ok := a.assets.Data[asset3d.GetUserAssetIDPair()]; !ok {
		return false, nil
	}

	if updateDB {
		if err := a.db.GetAssets3dDB().RemoveAssetByID(a.ctx, asset3d.GetUserAssetIDPair()); err != nil {
			return false, errors.WithMessage(err, "failed to update db")
		}
	}

	// TODO combine Asset3dID and UserID to create a unique key?
	delete(a.assets.Data, asset3d.GetUserAssetIDPair())
...
// TODO add userID
func (a *Assets3d) RemoveAsset3dByID(assetUserID universe.AssetUserIDPair, updateDB bool) (bool, error) {
	a.assets.Mu.Lock()
	defer a.assets.Mu.Unlock()

	if _, ok := a.assets.Data[assetUserID]; !ok {
		return false, nil
	}

	if updateDB {
		if err := a.db.GetAssets3dDB().RemoveAssetByID(a.ctx, assetUserID); err != nil {
			return false, errors.Errorf("failed to update db")
		}
	}

	delete(a.assets.Data, assetUserID)
...
func (a *Assets3d) Load() error {
	...
	for _, assetEntry := range entries {
		asset3d, err := a.CreateAsset3d(universe.AssetUserIDPair{
			AssetID: assetEntry.Asset3dID,
			UserID:  assetEntry.UserID,
		})
		if err != nil {
