...
type Assets3d struct {
	...
	cfg    *config.Config
	db     database.DB
	assets *generic.SyncMap[mid.ID, universe.Asset3d]
}

func NewAssets3d(db database.DB) *Assets3d {
	return &Assets3d{
		db:     db,
		assets: generic.NewSyncMap[mid.ID, universe.Asset3d](0),
	}
}
...
func (a *Assets3d) CreateAsset3d(asset3dID mid.ID) (universe.Asset3d, error) {
...
func (a *Assets3d) FilterAssets3d(predicateFn universe.Assets3dFilterPredicateFn) map[mid.ID]universe.Asset3d {
	return a.assets.Filter(predicateFn)
}

func (a *Assets3d) GetAsset3d(asset3dID mid.ID) (universe.Asset3d, bool) {
	asset, ok := a.assets.Load(asset3dID)
	return asset, ok
}

func (a *Assets3d) GetAssets3d() map[mid.ID]universe.Asset3d {
	a.assets.Mu.RLock()
	defer a.assets.Mu.RUnlock()

	assets := make(map[mid.ID]universe.Asset3d, len(a.assets.Data))
...
func (a *Assets3d) RemoveAssets3d(assets3d []universe.Asset3d, updateDB bool) (bool, error) {
	...
	if updateDB {
		ids := make([]mid.ID, 0, len(assets3d))
		for i := range assets3d {
...
func (a *Assets3d) RemoveAsset3dByID(asset3dID mid.ID, updateDB bool) (bool, error) {
	a.assets.Mu.Lock()
	defer a.assets.Mu.Unlock()
...
func (a *Assets3d) RemoveAssets3dByIDs(assets3dIDs []mid.ID, updateDB bool) (bool, error) {
	a.assets.Mu.Lock()
	defer a.assets.Mu.Unlock()
