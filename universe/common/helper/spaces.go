...
func SpaceTemplateFromMap(m map[string]any) (*SpaceTemplate, error) {
	...
	if err := utils.MapDecode(m, &template); err != nil {
		return nil, errors.WithMessage(err, "failed to decode map")
	}

	var convert func(in *spaceTemplate) *SpaceTemplate
	convert = func(in *spaceTemplate) *SpaceTemplate {
		out := SpaceTemplate{
			SpaceID:         in.SpaceID,
			SpaceName:       in.SpaceName,
			SpaceTypeID:     in.SpaceTypeID,
			ParentID:        in.ParentID,
			OwnerID:         in.OwnerID,
			Asset2dID:       in.Asset2dID,
			Asset3dID:       in.Asset3dID,
			Options:         in.Options,
			Position:        in.Position,
			Label:           in.Label,
			SpaceAttributes: make([]*entry.Attribute, len(in.SpaceAttributes)),
			Spaces:          make([]*SpaceTemplate, len(in.Spaces)),
		}

		for i := range in.SpaceAttributes {
			out.SpaceAttributes[i] = entry.NewAttribute(
				in.SpaceAttributes[i].AttributeID, &in.SpaceAttributes[i].AttributePayload,
			)
		}

		for i := range in.Spaces {
			out.Spaces[i] = convert(in.Spaces[i])
		}

		return &out
	}

	return convert(&template), nil
}
