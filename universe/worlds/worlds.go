...
type Worlds struct {
	...
	cfg    *config.Config
	db     database.DB
	worlds *generic.SyncMap[mid.ID, universe.World]
}

func NewWorlds(db database.DB) *Worlds {
	return &Worlds{
		db:     db,
		worlds: generic.NewSyncMap[mid.ID, universe.World](0),
	}
}
...
func (w *Worlds) CreateWorld(worldID mid.ID) (universe.World, error) {
...
func (w *Worlds) FilterWorlds(predicateFn universe.WorldsFilterPredicateFn) map[mid.ID]universe.World {
	return w.worlds.Filter(predicateFn)
}

func (w *Worlds) GetWorld(worldID mid.ID) (universe.World, bool) {
	world, ok := w.worlds.Load(worldID)
	return world, ok
}

func (w *Worlds) GetWorlds() map[mid.ID]universe.World {
	w.worlds.Mu.RLock()
	defer w.worlds.Mu.RUnlock()

	worlds := make(map[mid.ID]universe.World, len(w.worlds.Data))
