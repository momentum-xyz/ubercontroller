...
func (w *Worlds) apiGetOnlineUsers(c *gin.Context) {
	worldID, err := mid.Parse(c.Param("objectID"))
	if err != nil {
		err := errors.WithMessage(err, "Worlds: apiGetOnlineUsers: failed to parse world mid")
	...
	userIDs := make([]mid.ID, 0, len(users))
...
func (w *Worlds) apiWorldsGetObjectsWithChildren(c *gin.Context) {
	...
	objectID, err := mid.Parse(inQuery.ObjectID)
	if err != nil {
		err := errors.WithMessage(err, "Worlds: apiWorldsGetObjectsWithChildren: failed to parse object mid")
	...
	worldID, err := mid.Parse(c.Param("objectID"))
	if err != nil {
		err := errors.WithMessage(err, "Worlds: apiWorldsGetObjectsWithChildren: failed to parse world mid")
		api.AbortRequest(c, http.StatusBadRequest, "invalid_world_id", err, w.log)
		return
	}
...
func (w *Worlds) apiWorldsGetChildrenOptions(
	objects map[mid.ID]universe.Object, currentLevel int, maxLevel int,
...
func (w *Worlds) apiWorldsSearchObjects(c *gin.Context) {
	...
	if err != nil {
		err := errors.WithMessage(err, "Worlds: apiWorldsSearchObjects: failed to parse world mid")
		api.AbortRequest(c, http.StatusBadRequest, "invalid_world_id", err, w.log)
		return
	}
...
func (w *Worlds) apiWorldsFilterObjects(searchQuery string, world universe.World) (dto.SearchOptions, error) {
	predicateFn := func(objectID mid.ID, object universe.Object) bool {
...
func (w *Worlds) apiWorldsTeleportUser(c *gin.Context) {
	objectID, err := mid.Parse(c.Param("objectID"))
	if err != nil {
		err := errors.WithMessage(err, "Worlds: apiWorldsTeleportUser: failed to parse world mid")
		api.AbortRequest(c, http.StatusBadRequest, "invalid_world_id", err, w.log)
		return
	}
	...
	if err != nil {
		err = errors.WithMessage(err, "Worlds: apiWorldsTeleportUser: failed to get user mid from token")
	...
	if err != nil {
		err = errors.WithMessage(err, "Worlds: apiWorldsTeleportUser: failed to get user by mid")
