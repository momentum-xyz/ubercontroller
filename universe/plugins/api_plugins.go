...
func (p *Plugins) apiGetPlugins(c *gin.Context) {
	...
	ids := make([]umid.UMID, len(inQuery.IDs))
	for i := range inQuery.IDs {
		id, err := umid.Parse(inQuery.IDs[i])
		if err != nil {
			err = errors.WithMessagef(err, "Plugins: apiGetPlugins: failed to parse umid: %s", inQuery.IDs[i])
	...
	filterFn := func(pluginID umid.UMID, plugin universe.Plugin) bool {
		if len(ids) > 0 {
			var found bool
			for i := range ids {
				if pluginID == ids[i] {
	...
	var plugins map[umid.UMID]universe.Plugin
...
func (p *Plugins) apiGetPluginsMeta(c *gin.Context) {
	...
	for _, id := range inQuery.IDs {
		...
		if !ok {
			err = errors.Errorf("Plugins: apiGetPluginsMeta: failed to get plugin by umid: %s", pluginID)
...
func (p *Plugins) apiGetPluginsOptions(c *gin.Context) {
	...
	for _, id := range inQuery.IDs {
		...
		plugin, ok := p.GetPlugin(pluginID)
		if !ok {
			err = errors.Errorf("Plugins: apiGetPluginsOptions: failed to get plugin by umid: %s", pluginID)
			api.AbortRequest(c, http.StatusNotFound, "plugin_not_found", err, p.log)
			return
		}
