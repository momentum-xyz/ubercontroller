...
func (v *ActivityData) UnmarshalMUS(buf []byte) (int, error) {
	...
	if buf[i] == 0 {
		...
		i++
		{
			...
			(*v.BCTxHash) = string(buf[i : i+length])
			i += length
		}
	}
	if err != nil {
		return i, muserrs.NewFieldError("BCTxHash", err)
	}
	v.BCLogIndex = new(string)
	if buf[i] == 0 {
		i++
		v.BCLogIndex = nil
	} else if buf[i] != 1 {
		...
		i++
		{
			...
			(*v.BCLogIndex) = string(buf[i : i+length])
			i += length
		}
	}
	if err != nil {
		return i, muserrs.NewFieldError("BCLogIndex", err)
	}
	return i, err
}
