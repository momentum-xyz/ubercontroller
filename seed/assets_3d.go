...
func seedAssets3d(ctx context.Context, node universe.Node) error {
	...
	*/
	...
	for _, item := range items {
		...
		userAsset, err := node.GetAssets3d().CreateUserAsset3d(item.Asset3dID, umid.MustParse("00000000-0000-0000-0000-000000000003"), false)
		if err != nil {
			return errors.WithMessagef(err, "failed to create asset_3d: %s", item.Asset3dID)
		}
		...
		if isNewInstance {
			if err = asset.SetMeta(&baseMeta, false); err != nil {
				return errors.WithMessagef(err, "failed to set asset_3d meta: %s", item.Asset3dID)
			}
		}

		if err = userAsset.SetMeta(item.Meta, false); err != nil {
			return errors.WithMessagef(err, "failed to set user asset_3d meta: %s", item.Asset3dID)
		}
	}
